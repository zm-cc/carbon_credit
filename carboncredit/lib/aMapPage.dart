...
LatLng? l;
LatLng? l1;
List<LatLng> _trackPath = [];
//Iterable<Polyline> _polylines =new ;
...
class AMapPage extends StatelessWidget{
  ...
  Widget build(BuildContext context) {
    ...
    CameraPosition kInitialPosition = CameraPosition(
      target: latLng ?? const LatLng(31.22, 121.48),
      zoom: 18.0,
    );
    ...
    _timer1=Timer.periodic(const Duration(seconds:2),(timer){
      ...
      if (l!=null && l1==null) l1=l;
      if (isstart==false && l!=null && l1!=null && l!=l1)
      {
        l1=l;
      }
      if (isstart==true && l!=null && l1!=null && l!=l1) {
        List<LatLng> list = <LatLng>[];
        list.add(l!);
        list.add(l1!);
        distanceinmeter+=_getDistance(l!.latitude,l!.longitude,l1!.latitude,l1!.longitude);
        print("距离为");
    ...
    return AMapWidget(
      ...
      /*onLocationChanged: (argument) {
            ...
            target: LatLng(lat.value, long.value), zoom: 18, bearing: 0)),
           animated: true);
             },*/
    );
  }
...
class LocationInfo {
  ...
  LocationInfo(Map<String, Object> locationResult) {
    ...
    longitude = locationResult["longitude"] as double;
    if (latitude!=null && longitude!=null) {l=LatLng(latitude!,longitude!);}
